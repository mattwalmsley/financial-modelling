...
```math
V_1 = w_0 R_0 + \sum_{k=1}^n w_k R_k = w_0 R_0 + \mathbf{w}^T \mathbf{R}
```

where $w_0$ is the amount invested in the risk-free asset, $\mathbf{w} = (w_1, \ldots, w_n)^T$ is the vector of amounts invested in each risky asset, and $\mathbf{R} = (R_1, \ldots, R_n)^T$ is the vector of returns of the risky assets.

The variance of the portfolio return can then be written as

```math
\begin{aligned}
\text{Var}(V_1) &= \text{Var}(w_0 R_0 + \mathbf{w}^T \mathbf{R}) \\
&= \text{Var}(\mathbf{w}^T \mathbf{R}) \\
&= \mathbf{w}^T \Sigma \mathbf{w}
\end{aligned}
```

where we define the covariance matrix of the asset returns
...
Let us consider the case without a risk-free asset ($w_0 = 0$) and two simple scenarios:

**(a)** Assume that the risky assets are uncorrelated, that is, $\text{Cov}(R_i, R_j) = 0$ for $i \neq j$, and that the rates of return all have the same expected value, $\mathbb{E}[R_i] = \mu$ for all $i$, and the same variance, $\text{Var}(R_i) = \sigma^2$ for all $i$. If the weights $w_i = 1/n$ are all equal, we have

```math
\mathbb{E}[\mathbf{w}^T \mathbf{R}] = \sum_k w_k \mathbb{E}[R_k] = \sum_k \frac{1}{n} \mu = \mu
```

and

```math
\text{Var}(\mathbf{w}^T \mathbf{R}) = \sum_i \sum_j w_i w_j \text{Cov}(R_i, R_j) = \sum_i \frac{1}{n} \cdot \frac{1}{n} \text{Var}(R_i) = \frac{\sigma^2}{n}
```

Therefore, we can reduce the variance of the portfolio (the spread around the expected value) to zero for large $n$. This means diversification of your investments can reduce the risk.

**(b)** Assume again equal expected returns and variances of the returns, but now each pair of assets has the covariance

```math
\text{Cov}(R_i, R_j) = \alpha \sigma^2
```

that is, the pair covariance ($i \neq j$) is proportional to $\sigma^2$ with $\alpha \in [0, 1]$. Then we still have $\mathbb{E}[\mathbf{w}^T \mathbf{R}] = \mu$, but

```math
\begin{aligned}
\text{Var}(\mathbf{w}^T \mathbf{R}) &= \sum_i \sum_j w_i w_j \text{Cov}(R_i, R_j) = \frac{1}{n^2}\left(\sum_i \text{Var}(R_i) + \sum_{i \neq j} \text{Cov}(R_i, R_j)\right) \\
&= \frac{1}{n^2}\left(n\sigma^2 + n(n-1)\alpha\sigma^2\right) \\
&= \sigma^2\left(\frac{1 - \alpha}{n} + \alpha\right)
\end{aligned}
```

Here, we can maximally reduce the variance to $\alpha\sigma^2$. The conclusion is that correlations in assets make it more difficult to reduce the portfolio variance through diversification.

In general, diversification leads to a trade-off between lower expected rate of return and lower variance.

## Portfolio Diagrams

In the following we will also use the notation $\mu_i = \mathbb{E}[R_i]$ and $\sigma_{ij}^2 = \text{Cov}(R_i, R_j)$. We denote the mean and variance of the portfolio by

```math
\mu_p = \mathbb{E}[\mathbf{w}^T \mathbf{R}], \quad \sigma_p^2 = \text{Var}(\mathbf{w}^T \mathbf{R})
```

Consider a portfolio of two assets. We have thus
...
Without loss of generality, we can always set $V_0 = 1$, since all other values are simply multiples. For a strict budget constraint $V_0 = w_1 + w_2$ we can express the portfolio weights in the form $w_1 = 1 - \alpha$ and $w_2 = \alpha$, with $\alpha \in [0, 1]$ if shortselling is prohibited, i.e., $w_{1,2} \geq 0$. This means that all possible portfolios given by different weights of $w_1$ and $w_2$ under the strict budget constraint are simply described by choosing a value of $\alpha$. For $\alpha = 0$ everything is invested into asset 1, while for $\alpha = 1$ everything is invested into asset 2. For intermediate $\alpha$ we have a composition of investments into 1 and 2. Plotting now $\mu_p$ against $\sigma_p$ as a function of $\alpha$ we obtain a curve of all possible portfolios in the $\mu_p$-$\sigma_p$ plane. What is the shape of this curve? The answer is given by the following proposition.

**Proposition.** The curve in a $\mu$-$\sigma$ diagram defined by non-negative mixtures of two assets 1 and 2 lies within the triangular region defined by the two original assets and the point on the $\mu$ axis of height

```math
A = \frac{\mu_1 \sigma_{22} + \mu_2 \sigma_{11}}{\sigma_{11} + \sigma_{22}}
```

See the exercises for a proof. Assets 1 and 2 are prescribed in the diagram by their mean values $\mu_{1,2}$ and standard deviations $\sigma_1 = \sqrt{\sigma_{11}^2}$, $\sigma_2 = \sqrt{\sigma_{22}^2}$. Depending on the covariance $\sigma_{12}^2$ we obtain different (left convex) curves (parametrized by $\alpha$) that describe all possible portfolios consisting of the two assets. All possible curves for different covariances lie inside the triangle spanned by asset 1, 2 and the point $A$.

In general, we consider $n$ risky assets and assume that portfolios are constructed using any possible weighting for the $w_i$, under the strict budget constraint that $\sum w_i = V_0$. The set of points in the $\mu$-$\sigma$ diagram that correspond to these portfolios is called the feasible region. The feasible region has the following properties

(a) If there are at least 3 assets, not perfectly correlated and with different means, then the feasible region will be a two dimensional surface.

(b) The feasible region is convex to the left, that is, the straight line connecting any two points inside the feasible region does not cross the left boundary.
...
We have seen that an investor would ideally like to choose a portfolio that attains a high expected value, while keeping the variance small in order to reduce the risk. The portfolio value at time 1 is
...
```math
\mathbb{E}[V_1] = \mathbb{E}[w_0 R_0 + \mathbf{w}^T \mathbf{R}] = w_0 R_0 + \mathbf{w}^T \boldsymbol{\mu}
```

where $\boldsymbol{\mu} = (\mu_1, \ldots, \mu_n)^T$ is the vector of expected returns $\mu_i = \mathbb{E}[R_i]$ of the individual risky investments. The variance is

```math
\text{Var}(V_1) = \text{Var}(w_0 R_0 + \mathbf{w}^T \mathbf{R}) = \text{Var}(\mathbf{w}^T \mathbf{R}) = \mathbf{w}^T \Sigma \mathbf{w}
```
...
```math
\text{maximize} \quad w_0 R_0 + \mathbf{w}^T \boldsymbol{\mu} - \frac{c}{2V_0} \mathbf{w}^T \Sigma \mathbf{w}
```

subject to the budget constraint

```math
w_0 R_0 + \mathbf{w}^T \mathbf{1} \leq V_0
```

Here, the dimensionless constant $c > 0$ is called the trade-off parameter, which quantifies the investor's choice of trade-off between maximizing $\mathbf{w}^T \boldsymbol{\mu}$ and minimizing $\mathbf{w}^T \Sigma \mathbf{w}$. For example, an investor who does not care about risks would choose $c = 0$, while a risk-averse investor would choose a large value of $c$. Note that the factor $1/2V_0$ appears for convenience. The maximization yields the optimal portfolio weights $w_0^*$, $\mathbf{w}^*$, which tell the investor how much to invest in each of the assets given an initial budget $V_0$.

Instead of maximizing, we can minimize the function

```math
f(w_0, \mathbf{w}) = -w_0 R_0 - \mathbf{w}^T \boldsymbol{\mu} + \frac{c}{2V_0} \mathbf{w}^T \Sigma \mathbf{w}
```

since $f$ is just the negative. The solution $\mathbf{w}$ is thus identical for both. The minimization is more convenient for us, because $f$ is a convex function and we can apply results from general convex optimization theory (see below). Before we can calculate the solution $\mathbf{w}$ of the portfolio optimization problem, we first need to understand how to calculate extrema subject to constraints.

## Excursion: Finding extrema subject to constraints

Strict equality constraint

We are interested in finding the extremum of a function $f(\mathbf{x})$, where $\mathbf{x} = (x_1, x_2, \ldots, x_n)^T$, subject to a constraint on $\mathbf{x}$. The constraint can in general be expressed in the form $g(\mathbf{x}) = g_0$. Here, both $f$ and $g$ are functions $\mathbb{R}^n \to \mathbb{R}$. For concreteness, let us consider as a simple example the following two dimensional problem:
...
**(a) Substitution method:** Here, you solve the constraint for either $x$ or $y$ and substitute this result into the function $f$. This means you obtain a new function $\tilde{f}$ which only depends on one variable and the minimum of this function is the constrained minimum of $f$. In the example, you need to substitute either $y = 1 - x$ or $x = 1 - y$. Using the former we find the minimum of
...
**(c) Method of Lagrange multipliers:** We can translate this graphical observation into mathematical equations as follows. The condition of a tangent point is equivalent to requiring that the gradient of $f$ and the normal vector on $g$ are parallel. This means that
...
The important question is now, what happens when the constraint becomes an inequality constraint? If the equality constraint is relaxed to an inequality constraint $x + y \leq 1$, the minimization is over all values in the feasible area. If the vectors $-\nabla f$ and $\nabla g$ at the tangent point show in opposite directions, i.e., $\lambda < 0$, we see that the unconstrained global minimum becomes optimal, since it is the global minimum of $f$ and thus always lower than the constrained one. However, depending on $f$ there is also a second possible scenario. If the function $f$ is such that both vectors point into the same direction at the constrained minimum, i.e., $\lambda > 0$, the constrained minimum remains optimal due to the convexity of $f$.
...
Let us come back to the portfolio optimization problem and first consider the strict budget constraint without a risk-free asset ($w_0 = 0$)

```math
g(\mathbf{w}) = \mathbf{w}^T \mathbf{1} = V_0
```
...
```math
\mathcal{L}(\mathbf{w}) = f(\mathbf{w}) + \lambda(g(\mathbf{w}) - V_0) = -\mathbf{w}^T \boldsymbol{\mu} + \frac{c}{2V_0} \mathbf{w}^T \Sigma \mathbf{w} + \lambda(\mathbf{w}^T \mathbf{1} - V_0)
```
...
where we introduce the vectors $\mathbf{1} = (1, \ldots, 1)^T$ and $\mathbf{0} = (0, \ldots, 0)^T$. This together with the strict budget constraint provide $n+1$ equations for the $n+1$ unknowns: $w_1, w_2, \ldots, w_n, \lambda$. We can first solve by matrix inversion to obtain
...
```math
V_0 = \frac{V_0}{c} \left( \mathbf{1}^T \Sigma^{-1} \boldsymbol{\mu} - \lambda \mathbf{1}^T \Sigma^{-1} \mathbf{1} \right)
```

and thus

```math
\lambda = \frac{\mathbf{1}^T \Sigma^{-1} \boldsymbol{\mu} - c}{\mathbf{1}^T \Sigma^{-1} \mathbf{1}}
```

The solution for $\mathbf{w}$ is thus

```math
\mathbf{w} = \frac{V_0}{c} \Sigma^{-1} \left( \boldsymbol{\mu} - \frac{\mathbf{1}^T \Sigma^{-1} \boldsymbol{\mu} - c}{\mathbf{1}^T \Sigma^{-1} \mathbf{1}} \mathbf{1} \right)
```
...
```math
g(\mathbf{w}) = \mathbf{w}^T \mathbf{1} \leq V_0
```
...
```math
\mathbf{1}^T \Sigma^{-1} \boldsymbol{\mu} \leq c
```
...
```math
\mathbf{1}^T \mathbf{w} = \frac{V_0}{c} \mathbf{1}^T \Sigma^{-1} \boldsymbol{\mu} \leq V_0
```

because of the condition above. According to the KKT Proposition, the unconstrained solution is thus indeed an optimal solution for the regime $c \geq \mathbf{1}^T \Sigma^{-1} \boldsymbol{\mu}$. Overall, we obtain two different solutions depending on the trade-off parameter $c$. It can be summarized as follows:

**The optimal solution** to the portfolio optimization problem without a risk-free asset ($w_0 = 0$) is given by

```math
\mathbf{w} = \frac{V_0}{c} \Sigma^{-1} \left( \boldsymbol{\mu} - \left( \frac{\mathbf{1}^T \Sigma^{-1} \boldsymbol{\mu} - c}{\mathbf{1}^T \Sigma^{-1} \mathbf{1}} \right)^{+} \mathbf{1} \right)
```
...
```math
g(w_0, \mathbf{w}) = w_0 + \mathbf{w}^T \mathbf{1} = V_0
```

In order to solve the optimization problem, we use the method of Lagrange multipliers. For this we introduce the function

```math
\mathcal{L}(w_0, \mathbf{w}) = f(w_0, \mathbf{w}) + \lambda(g(w_0, \mathbf{w}) - V_0) = -w_0 R_0 - \mathbf{w}^T \boldsymbol{\mu} + \frac{c}{2V_0} \mathbf{w}^T \Sigma \mathbf{w} + \lambda(w_0 + \mathbf{w}^T \mathbf{1} - V_0)
```
...
```math
g(w_0, \mathbf{w}) = w_0 + \mathbf{w}^T \mathbf{1} \leq V_0
```
...
(c) There is a unique value of the trade-off parameter, such that the optimal portfolio is fully invested in the risky assets only. Denote this value by $c^*$. We can determine $c^*$ by solving the equation $\mathbf{w}^T \mathbf{1} = V_0$, because then the amount invested in the risk-free asset is $w_0 = V_0 - \mathbf{w}^T \mathbf{1} = 0$. This yields

```math
c^* = \mathbf{1}^T \Sigma^{-1} (\boldsymbol{\mu} - R_0 \mathbf{1})
```
...
```math
\begin{aligned}
\mu_p(c) &= \mathbb{E}[V_1] = \mathbf{w}^T \boldsymbol{\mu} \\
\sigma_p(c) &= \sqrt{\text{Var}(V_1)} = \sqrt{\mathbf{w}^T \Sigma \mathbf{w}}
\end{aligned}
```
...
The plot of the resulting portfolio line shows that it curves down for $c < \mathbf{1}^T \Sigma^{-1} \boldsymbol{\mu}$ (black line), while for $c > \mathbf{1}^T \Sigma^{-1} \boldsymbol{\mu}$ it is a straight line (blue line). From the solution we see that for $c \to \infty$, $\mathbf{w} \to \mathbf{0}$. The interpretation is that for larger risk averseness (larger $c$), the optimal solution is to invest less and less at all.

(b) If we include a risk-free asset, we have the solution

```math
\mathbf{w} = \frac{V_0}{c} \Sigma^{-1} (\boldsymbol{\mu} - R_0 \mathbf{1}), \quad w_0 = V_0 - \mathbf{w}^T \mathbf{1}
```
...
```math
\begin{aligned}
\mu_p(c) &= \mathbb{E}[V_1] = w_0 R_0 + \mathbf{w}^T \boldsymbol{\mu} \\
\sigma_p(c) &= \sqrt{\text{Var}(V_1)} = \sqrt{\mathbf{w}^T \Sigma \mathbf{w}}
\end{aligned}
```
...
```math
\mathbf{w} = \alpha \mathbf{w}_t, \quad w_0 = V_0 - \alpha \mathbf{w}_t^T \mathbf{1}
```
...
(c) A portfolio with $\mathbf{w} = \alpha \mathbf{w}_t$ in risky assets and $w_0 = V_0 - \alpha \mathbf{w}_t^T \mathbf{1}$ in the risk-free asset has expected return

```math
\mu_p = (V_0 - \alpha \mathbf{w}_t^T \mathbf{1}) R_0 + \alpha \mathbf{w}_t^T \boldsymbol{\mu}
```

and risk

```math
\sigma_p = \sqrt{\alpha \mathbf{w}_t^T \Sigma \alpha \mathbf{w}_t} = \alpha \sqrt{\mathbf{w}_t^T \Sigma \mathbf{w}_t}
```
...
```math
\begin{aligned}
\mu_p &= V_0 R_0 + \alpha \mathbf{w}_t^T (\boldsymbol{\mu} - R_0 \mathbf{1}) \\
&= V_0 R_0 + \sigma_p \frac{\mathbf{w}_t^T (\boldsymbol{\mu} - R_0 \mathbf{1})}{\sqrt{\mathbf{w}_t^T \Sigma \mathbf{w}_t}} \\
&= V_0 R_0 + \frac{\mathbf{w}_t^T \boldsymbol{\mu} - R_0 V_0}{\sqrt{\mathbf{w}_t^T \Sigma \mathbf{w}_t}} \sigma_p
\end{aligned}
```

We have shown that the efficient frontier in the $\mu$-$\sigma$ diagram is a straight line.

It is important to note that the mathematical solutions of the portfolio optimization problem always assume $w_i \in \mathbb{R}$, which means in economic terms that shortselling is allowed. On the other hand, the portfolio diagrams discussed qualitatively earlier assume that shortselling is prohibited. How are the diagrams related? With the help of a computer, we can clarify the situation. We use the four risky assets specified by $\boldsymbol{\mu}, \Sigma$ and now simulate all possible portfolios by generating random numbers for the portfolio weights $w_i$. Each set of random numbers $w_1, w_2, w_3, w_4$ can be used as $\mathbf{w}$ in $\mu_p = \mathbf{w}^T \boldsymbol{\mu}$, $\sigma_p = \sqrt{\mathbf{w}^T \Sigma \mathbf{w}}$ and corresponds to a single point in the portfolio diagram. Sampling a large number of such $\mathbf{w}$ values for a given constraint yields the simulated feasible regions. We observe the following:
...
Consider the following portfolio optimization problem including a risk-free asset: $V_1 = w_0 R_0 + \mathbf{w}^T \mathbf{R}$

```math
\begin{aligned}
\text{maximize} \quad & \mathbb{E}[V_1] = w_0 R_0 + \mathbf{w}^T \boldsymbol{\mu} \\
\text{subject to} \quad & \text{Var}(V_1) = \mathbf{w}^T \Sigma \mathbf{w} \leq \sigma_0^2 V_0^2, \quad w_0 + \mathbf{w}^T \mathbf{1} \leq V_0
\end{aligned}
```
...
```math
\mathbf{w} = \frac{\sigma_0 V_0 \, \Sigma^{-1} (\boldsymbol{\mu} - R_0 \cdot \mathbf{1})}{\sqrt{(\boldsymbol{\mu} - R_0 \cdot \mathbf{1})^T \Sigma^{-1} (\boldsymbol{\mu} - R_0 \cdot \mathbf{1})}}
```
...
```math
\begin{aligned}
\text{minimize} \quad & \tfrac{1}{2} \text{Var}(V_1) = \tfrac{1}{2} \mathbf{w}^T \Sigma \mathbf{w} \\
\text{subject to} \quad & \mathbb{E}[V_1] = w_0 R_0 + \mathbf{w}^T \boldsymbol{\mu} \geq \mu_0 V_0, \quad w_0 + \mathbf{w}^T \mathbf{1} \leq V_0
\end{aligned}
```
...
```math
\mathbf{w} = \frac{V_0 (\mu_0 - R_0) \, \Sigma^{-1} (\boldsymbol{\mu} - R_0 \cdot \mathbf{1})}{(\boldsymbol{\mu} - R_0 \cdot \mathbf{1})^T \Sigma^{-1} (\boldsymbol{\mu} - R_0 \cdot \mathbf{1})}
```
...
```math
\frac{\partial}{\partial w_l} \left\{ \tfrac{1}{2} \mathbf{w}^T \Sigma \mathbf{w} + \lambda_1 (\mu_0 V_0 - w_0 R_0 - \mathbf{w}^T \boldsymbol{\mu}) + \lambda_2 (w_0 + \mathbf{w}^T \mathbf{1} - V_0) \right\}
```

for $l = 1, \ldots, n$. Note that all constraints have to be included in the form $g_j(x) - g_{j,0} \leq 0$ leading to the term $+\lambda_1 (\mu_0 V_0 - w_0 R_0 - \mathbf{w}^T \boldsymbol{\mu})$. We also need to calculate the derivative with respect to $w_0$. Setting the derivatives to zero leads to the system of equations in vector notation
...
```math
\begin{aligned}
w_0 R_0 + \mathbf{w}^T \boldsymbol{\mu} &= \mu_0 V_0 \\
w_0 + \mathbf{w}^T \mathbf{1} &= V_0
\end{aligned}
```

In order to determine $\lambda_1$ we first eliminate $w_0$ from the two constraint equations, which leads to: $\mathbf{w}^T \boldsymbol{\mu} - \mathbf{w}^T \mathbf{1} R_0 = \mathbf{w}^T (\boldsymbol{\mu} - \mathbf{1} R_0) = V_0 (\mu_0 - R_0)$. Substituting the solution $\mathbf{w}$ into this equation leads to

```math
\lambda_1 = \frac{V_0 (\mu_0 - R_0)}{(\boldsymbol{\mu} - R_0 \cdot \mathbf{1})^T \Sigma^{-1} (\boldsymbol{\mu} - R_0 \cdot \mathbf{1})}
```
