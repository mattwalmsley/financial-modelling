...
from bbg_data.enums import BloombergField, OptionType, ServiceType
from bbg_data.models import (
    HistoricalDataPoint,
    OptionChainFilter,
    OptionContract,
    OptionMarketData,
...
from bbg_data.session import BloombergSession
...
class OptionChainFetcher:
    ...
    def get_historical_option_data(
        ...
        from bbg_data.requests import to_dataframe
...
def fetch_nvidia_options(
    expiry_date: str | date,
    fields: list[BloombergField] | None = None,
    strike_range: tuple[float | None, float | None] = (None, None),
) -> pd.DataFrame:
    """
    Convenience function to fetch NVIDIA option chain data.
    ...
    Example:
        >>> df = fetch_nvidia_options("11/21/25")
        >>> calls = df[df['OPT_PUT_CALL'] == 'CALL']
    """
