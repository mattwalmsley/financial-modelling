...
This module provides a set of functions to compute various risk-adjusted performance measures, primarily focused on the Sharpe Ratio and its statistical adjustments.

Functions:
----------
- estimated_sharpe_ratio(returns):
    Computes the estimated Sharpe Ratio based on a given series of returns.

- ann_estimated_sharpe_ratio(returns=None, periods=261, *, sr=None):
    Calculates the annualized estimated Sharpe Ratio given a series of returns and the number of periods.

- estimated_sharpe_ratio_stdev(returns=None, sr=None):
    Computes the standard deviation of the estimated Sharpe Ratio based on the series of returns.

- probabilistic_sharpe_ratio(returns=None, sr_benchmark=0.0, *, sr=None, sr_std=None):
    Calculates the Probabilistic Sharpe Ratio (PSR), which determines the probability that an observed Sharpe Ratio is statistically significant relative to a benchmark.

- min_track_record_length(...):
    Determines the minimum track record length required to achieve a desired level of statistical significance for a given Sharpe Ratio.

- num_independent_trials(trials_returns=None, *, m=None, p=None):
    Estimates the number of independent trials given a set of returns, the number of trials, and the probability of success.

- expected_maximum_sr(...):
    Computes the expected maximum Sharpe Ratio that can be observed by randomly selecting from multiple trials.

- deflated_sharpe_ratio(...):
    Calculates the Deflated Sharpe Ratio (DSR), which adjusts for the inflation in the Sharpe Ratio due to selection bias.

Dependencies:
-------------
- numpy as np
- pandas as pd
- scipy.stats as scipy_stats

This module is part of a larger library focusing on portfolio analysis and risk management.
"""
...
def estimated_sharpe_ratio_stdev(
    ...
    if kurtosis is None:
        kurtosis = pd.Series(
            scipy_stats.kurtosis(_returns, fisher=False), index=_returns.columns
        )
    if sr is None:
        sr = estimated_sharpe_ratio(_returns)

    sr_std = np.sqrt(
        (1 + (0.5 * sr**2) - (skew * sr) + (((kurtosis - 3) / 4) * sr**2)) / (n - 1)
    )
